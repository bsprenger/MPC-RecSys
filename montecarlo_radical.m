%% Monte Carlo Simulation for Model-free vs. Model-based Approaches with a radical user
clear
close all
clc
...
for trial=1:num_trials
    ...
    radical_user = randi([1, num_users]);
    
    W = generateRowStochasticMatrix(num_users,num_users+1);
    lambdas = rand([num_users 1]).*0.1;
    lambdas(radical_user) = 1; % set one user to be 100% biased
    Lambda = diag(lambdas);
    ...
    improvement_mpc_over_mf = steady_state_mf_cost - steady_state_mpc_cost;
    percentage_improvement_mpc_over_mf = ((steady_state_mf_cost - steady_state_mpc_cost) / steady_state_mf_cost) * 100;
    ...
    results(trial).improvement_mpc_over_mf = improvement_mpc_over_mf;
    results(trial).percentage_improvement_mpc_over_mf = percentage_improvement_mpc_over_mf;
...
save('monte_carlo_radical_user_data.mat','results');